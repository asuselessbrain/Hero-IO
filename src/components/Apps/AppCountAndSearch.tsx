...
const AppCountAndSearch = ({ searchTerm, setSearchTerm, filteredCount }: { searchTerm: string, setSearchTerm: React.Dispatch<React.SetStateAction<string>>, filteredCount: number }) => {
    return (
        <div className="flex flex-col md:flex-row items-center justify-between gap-2">
            <p className="text-2xl font-semibold flex-1 text-black">({filteredCount}) Apps Found</p>
            <div className="relative mt-4">
                ...
                <input
                    ...
                    className="w-full pl-10 p-3 rounded border-2 text-black border-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
            </div>
        </div>
    );
